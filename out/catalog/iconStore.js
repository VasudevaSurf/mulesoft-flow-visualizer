...
const coreIcons_1 = require("./coreIcons");
/**
 * Sanitizes and manages icons for both core and dynamic connectors.
 */
class IconStore {
    static ensureCoreIcons() {
        if (this.initialized) {
            return;
        }
        this.initialized = true;
        for (const [id, svg] of Object.entries(coreIcons_1.CORE_ICONS)) {
     ...
     */
    static sanitizeSvg(rawSvg, iconId) {
        if (!rawSvg) {
            return '';
        }
        try {
    ...
    static registerSymbol(iconId, symbolXml) {
        this.sanitizedIcons.set(iconId, symbolXml);
    }
    static getSymbol(iconId) {
...
IconStore.sanitizedIcons = new Map(); // iconId -> SVG symbol
IconStore.initialized = false;
